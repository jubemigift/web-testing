...
function initApp() {
    ...
    document.addEventListener('click', (e) => {
        if (!e.target.closest('.category-card') && !e.target.closest('.location-dropdown')) {
            closeAllDropdowns();
        }
    });
...
function loadCategories() {
    ...
    categoriesGrid.innerHTML = categoryData.map((category, index) => `
        <div class="category-card" style="animation-delay: ${index * 0.1}s">
            ...
            <div class="category-dropdown" id="dropdown-${category.name.replace(/\s+/g, '-').toLowerCase()}" onclick="event.stopPropagation()">
                <div class="dropdown-header">
                    <h4>Popular in ${category.name}</h4>
                </div>
                <div class="dropdown-items">
                    ${category.items.map((item, itemIndex) => `
                        <div class="dropdown-item" style="animation-delay: ${itemIndex * 0.1}s" onclick="event.stopPropagation(); showCustomizeModal('${item.id}')">
                ...
                <div class="dropdown-footer">
                    <button onclick="event.stopPropagation(); navigateTo('menu')">View All ${category.name} →</button>
                </div>
            </div>
        </div>
    `).join('');
...
function closeAllDropdowns() {
    document.querySelectorAll('.category-dropdown.active').forEach(dropdown => {
        dropdown.classList.remove('active');
    });
    document.querySelectorAll('.location-dropdown-menu.active').forEach(dropdown => {
        dropdown.classList.remove('active');
    });
            ...
            <div class="menu-item-content">
                ...
                <div class="menu-item-actions">
                    ...
                    ${(item.variants || item.addOns) ? `
                        <button class="btn btn-outline btn-sm" onclick="event.stopPropagation(); showCustomizeModal('${item.id}')" ${!item.available ? 'disabled' : ''}>
                            Customize
                        </button>
                    ` : ''}
                </div>
...
function updateCartDisplay() {
    ...
    cartFooter.innerHTML = `
        ...
        <div class="location-selection-section" id="locationSelectionSection">
            ...
            <div class="location-dropdown" onclick="toggleLocationDropdown()">
                ...
                <div class="location-dropdown-menu" id="locationDropdownMenu" onclick="event.stopPropagation()">
...
function renderLocationDropdownItems() {
    ...
    if (addresses.length === 0) {
        return `
            <div class="dropdown-empty">
                ...
                <button class="btn btn-primary btn-sm" onclick="event.stopPropagation(); showAddressModal()">Add Address</button>
            </div>
        `;
    }
    ...
    return addresses.map(address => `
        <div class="location-dropdown-item ${selectedAddress?.id === address.id ? 'selected' : ''}" 
             onclick="event.stopPropagation(); selectLocation('${address.id}')">
...
function showCustomizeModal(itemId) {
    ...
    customizeContent.innerHTML = `
        <div class="customize-item-info">
            <img src="${item.imageUrl}" alt="${item.name}" style="width: 100%; height: 200px; object-fit: cover; border-radius: 0.5rem; margin-bottom: 1rem;">
            <h3>${item.name}</h3>
                ...
                <div class="customize-options">
                    ${item.variants.map((variant, index) => `
                        <div class="customize-option">
                            <input type="radio" id="variant-${index}" name="variant" value="${variant.name}" data-price="${variant.priceDelta}" ${index === 0 ? 'checked' : ''} onchange="updateCustomizeTotal()">
                            <label for="variant-${index}">
                                ${variant.name}
                                <span class="option-price">${variant.priceDelta > 0 ? '+' : ''}${formatCurrency(variant.priceDelta)}</span>
                            </label>
                        </div>
                    `).join('')}
                </div>
            ...
            <div class="customize-section">
                ...
                <div class="customize-options">
                    ${item.addOns.map((addon, index) => `
                        <div class="customize-option">
                            <input type="checkbox" id="addon-${index}" name="addons" value="${addon.name}" data-price="${addon.price}" onchange="updateCustomizeTotal()">
        ...
        <div class="quantity-section" style="margin: 1.5rem 0;">
            <h4>Quantity</h4>
            <div style="display: flex; align-items: center; gap: 1rem;">
                <button type="button" class="quantity-btn" onclick="updateQuantity(-1)">
                    <i data-lucide="minus"></i>
                </button>
                <span id="customizeQuantity" class="quantity-display">1</span>
                <button type="button" class="quantity-btn" onclick="updateQuantity(1)">
                    <i data-lucide="plus"></i>
                </button>
            </div>
        ...
        <div class="form-actions" style="margin-top: 1.5rem;">
            <button class="btn btn-primary btn-lg" style="width: 100%;" onclick="addCustomizedToCart()">
                Add to Cart
            </button>
        </div>
    `;
    
    // Initialize icons and total
    lucide.createIcons();
...
function updateCustomizeTotal() {
    ...
    const quantity = parseInt(document.getElementById('customizeQuantity')?.textContent || 1);
    total *= quantity;
...
function updateQuantity(change) {
    const quantityElement = document.getElementById('customizeQuantity');
...
function addCustomizedToCart() {
    ...
    let totalPrice = customizingItem.basePrice;
    ...
    if (selectedVariant) {
        ...
        totalPrice += parseFloat(selectedVariant.dataset.price || 0);
    }
    ...
    showToast(`${customizingItem.name} added to cart!`, 'success');
